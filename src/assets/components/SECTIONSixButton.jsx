...
export default function AnimatedButton() {
  ...
  return (
    <div className=" mt-3.5 w-[420px] h-[65px] text-white flex items-center justify-start group overflow-hidden rounded-md bg-transparent">
      {/* Border Animation */}
      {showButton && (
        <div className="absolute inset-0  rounded-md animate-borderSweep z-0"></div>
      )}
      {/* Button */}
      {showButton && (
        <button className="relative flex items-center justify-center w-full font-sans font-extralight text-[21px] px-6 z-10 overflow-hidden bg-transparent">
          {/* Arrow Icon */}
          <span className="absolute left-[120px] opacity-100 transition-all duration-300 ease-in-out group-hover:left-[calc(100%-100px)] group-hover:opacity-100 z-0">
            <svg
              className="w-6 h-7"
